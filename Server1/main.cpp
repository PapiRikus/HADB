...
int main(int argc, char** argv) {
 ...
 regex creata("CREATE TABLE ([a-zA-Z0-9]+) \\(([a-zA-Z0-9]+( INT| BOOL| CHAR| STRING\\([0-9]+\\))+( REFERENCE\\([a-zA-Z0-9]+\\.[a-zA-Z0-9]+\\))?(, [a-zA-Z0-9]+ (INT|BOOL|CHAR|STRING\\([0-9]+\\))+( REFERENCE\\([a-zA-Z0-9]+\\.[a-zA-Z0-9]+\\))?)*)+(, PRIMARY KEY ([a-zA-Z]+))?");   
...
//    regex create("CREATE TABLE ([a-zA-Z0-9]+) \\(([a-zA-Z0-9]+( INT| BOOL| CHAR| STRING\\([0-9]+\\))+( REFERENCE\\([a-zA-Z0-9]+\\.[a-zA-Z0-9]+\\))?(,[a-zA-Z0-9]+ (INT|BOOL|CHAR|STRING\\([0-9]+\\))+( REFERENCE\\([a-zA-Z0-9]+\\.[a-zA-Z0-9]+\\))?)*)+(, PRIMARY KEY ([a-zA-Z]+))?\))"; // entire match will be 2 numbers
  ...
  //  cout<<"dsdfs"<<regex_match(query,selectjoin);
    ...
    if (regex_match(query,create)){
        cout<<"es verdad"<<endl;
    }
    std::regex_search(query,m,create);        
    ...
    data1.append("tipo");
    data1.append(medio);
    //cout<<m.str(4)<<m.str(4).compare("WHERE")<<" WHERE"<<endl;
    string where="WHERE";
    if (m.str(4)=="WHERE"){
        data1.append("WHERE");
        data1.append(despues);
